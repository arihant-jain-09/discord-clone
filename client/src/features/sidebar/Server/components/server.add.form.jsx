...
const AddServer = ({handleClose}) => {
  ...
  async function uploadImage(file) {
    ...
    const res = await fetch(
      `https://api.cloudinary.com/v1_1/${process.env.REACT_APP_CLOUD_ID}/image/upload`,
      {
        method: "POST",
        body: data
      }
  ...
  return (
    <>
      <div className="addServer">
        <div className="addServer__heading">Customize your server</div>
        <div className="addServer__subheading">Give your new server a personality with a name and an icon. You can always change it later.</div>
          <div className="addServer__add">
          <input accept="image/*" id="icon-button-file" type="file" onChange={handleFileChange} disabled={uploadingImg}/>
          ...
          {/* <Image cloudName="dv17tob3g" secure='true' upload_preset="my_unsigned_preset" publicId="IMG_20211013_210027_nn8zbe.jpg">
            <Transformation width="100" height="100" radius="max" crop="fill" />
          </Image> */}
          <div className="addServer__p">SERVER NAME</div>
          <div className="addServer__inputContainer">
            <input type="text" placeholder="my new server" onChange={(e)=>setFormData({...formData,server_name:e.target.value})} value={formData.server_name}/>
          </div>
        <div className="addServer__guidelines">By creating a server, you agree to Discord's Community Guidelines</div>
        <div className="addServer__buttons">
          <div className="addServer__buttons-back" onClick={handleClose}>Back</div>
          <div className="addServer__buttons-create" onClick={handleSubmit} disabled={uploadingImg}>Create</div>
        </div>
      </div>
    </>
  )
